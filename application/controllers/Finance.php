...
//        echo "<pre>"; print_r($data['registered_student']); exit;
        ...
        $this->load->view('admin/finance/resident_bill/resident_bill_index', $data);
    }
    ...
    /*
      * @methodName residentBillGenetareShow
      * @access
      * @param  none
    ...
    function residentBillAllShow()
    {

        $data['contentTitle'] = 'Resident Bill';
        $data['breadcrumbs'] = array(
            'Admin' => '#',
            'Resident Bill' => '#',
            );
        ...
        $data['content_view_page'] = 'admin/finance/resident_bill/resident_bill_index';

        //var_dump($data['resident']); 
        $this->admin_template->display($data);
   ...
   /*
     * @methodName residentBillInsert()
     * @access
     * @param  none
     * @return Mixed Template
     */

   function residentBillInsert()
   {
    $data["ac_type"] = 1;
    ...
    $this->load->view('admin/finance/resident_bill/add_resident_bill', $data);
}


       /*
     * @methodName createResidentBill()
     * @access
     * @param  none
     * @return status
     */

       function createResidentBill()
       {
        $AC_NO = $this->input->post('AC_NO');
        $AMOUNT = $this->input->post('AMOUNT');
        $SESSION_ID = $this->input->post('SESSION_ID');
        $RESEDENT_ID = $this->input->post('RESEDENT_ID'); 
        $MONTH = $this->input->post('BILLING_MONTH');
        $BILLING_MONTH=date('Y-m-d', strtotime('01-' . $MONTH));

        //var_dump($AMOUNT); die();
        $check_unique_value =array(
            'RESEDENT_ID' => $RESEDENT_ID , 
            'BILLING_MONTH' => $BILLING_MONTH
            );

        $result_check_unique_value = $this->utilities->hasInformationByThisId('fn_resident_bill_mst',$check_unique_value);
       

     
        if( $result_check_unique_value === FALSE){
                ...
                $RESIDENT_BILL_MST_ID=$this->utilities->insert('fn_resident_bill_mst',$resident_bill);


                    if ($RESIDENT_BILL_MST_ID) { 
                        for ($i = 0; $i < sizeof($AC_NO); $i++) {
                            $residentChargeRate = array(
                                'AC_NO' => $AC_NO[$i],
                                'AMOUNT' => $AMOUNT[$i],
                                'RESIDENT_BILL_MST_ID' => $RESIDENT_BILL_MST_ID, 
                                'CREATED_BY' => $this->user["USER_ID"]
                                );

                            //print_r($residentChargeRate);
                            $this->utilities->insertData($residentChargeRate, 'fn_resident_bill_chd');
                        }
                    }

                 echo "<div class='alert alert-success'>Resident Bill Create</div>";
            }
        else{
                 echo "<div class='alert alert-success'>Resident Bill Dulicate Value</div>";
    ...
    /*
     * @methodName residentBillList()
     * @access
     * @param  none
     * @return Mixed Template
     */

    function residentBillList()
    {
        $data["previlages"] = $this->checkPrevilege("setup/faculty");
       // $data['resident'] = $this->utilities->findAllFromView('fn_resident_bill'); // select all data from  Resident
        $data['resident'] = $this->finance_model->residentBillInformation();
        $this->load->view("admin/finance/resident_bill/resident_bill_list", $data);
    }


     /*
     * @methodName residentBillUpdate()
     * @access
     * @param  none
     * @return status
     */

     function residentBillUpdate()
     {
        $data["ac_type"] = 2;
        $id = $this->input->post('param'); // RESEDENT ID
       // var_dump($id);
        $data['resident_building'] = $this->utilities->findAllByAttribute('sa_building', array('BUILDING_TYPE' => 347));
        $data['ins_session'] = $this->utilities->academicSessionList();
        $data['Resident'] = $this->utilities->findByAttribute('fn_resident_bill', array('RESIDENT_BILL_ID' => $id)); // select all data from RESEDENT where RESEDENT id
    ...
    /*
     * @methodName updateResidentBill()
     * @access
     * @param  none
     * @return 
     */

    function updateResidentBill()
    {

        $RESIDENT_BILL_ID = $this->input->post('txtFacultyId'); 
        $SESSION_ID = $this->input->post('SESSION_ID');
        $RESEDENT_ID = $this->input->post('RESEDENT_ID'); 
        $BILLING_MONTH = $this->input->post('BILLING_MONTH'); 
        $ELECTRIC_BILL = $this->input->post('ELECTRIC_BILL'); 
        ...
        if (empty($check)) {// if Faculty name available
         ...
         $resident_bill = array(
            ...
            if ($this->utilities->updateData('fn_resident_bill', $resident_bill, array('RESIDENT_BILL_ID' => $RESIDENT_BILL_ID))) { // if data update successfully
                echo "<div class='alert alert-success'>Resident Bill Update successfully</div>";
            } else { // if data update failed
                echo "<div class='alert alert-danger'>Resident Bill Update failed</div>";
            }
        } else {// if faculty name not available
            echo "<div class='alert alert-danger'>Resident Bill Already Exist</div>";
        }
    }


    /*
     * @methodName residentBillById()
     * @access
     * @param  none
     * @return 
     */

    function residentBillById()
    {
        $RESIDENT_BILL_ID = $this->input->post('param'); // Resident Bill id
        $data["previlages"] = $this->checkPrevilege("setup/faculty");
        $data['row'] = $this->utilities->findByAttribute('fn_resident_bill', array('RESIDENT_BILL_ID' => $RESIDENT_BILL_ID)); 
        // select all data from Resident Bill where Resident Bill id
