...
foreach ($chargeId as $charge) {
   $charge_id[] = $charge->PRTCULR_NO;
}
/*echo "<pre>";
print_r($expenses);
exit();*/
if (!empty($expenses)):
    ...
    foreach ($expenses as $expense):
        ...
        <li>
            ...
            if (empty($dueAmt)) {
                ?>
                <input type="hidden" id="txtIsFirstTime" name="txtIsFirstTime" value="1"/>
                <input type="checkbox" class="expense" id="checkbox<?php echo $i; ?>"
                       data-amt="<?php echo $expense->AMOUNT; ?>"
                       value="<?php echo $expense->RATE_ID; ?>" data-pro="<?php echo $expense->PROGRAM_ID ?>"
                       data-sem="<?php echo $expense->SESSION_ID ?>">
            <?php
            } else {                    
                //echo $chargeId[$j]->PRTCULR_NO;
                if(in_array($expense->RATE_ID, $charge_id)){
                    ?>
                    <input type="hidden" id="txtIsFirstTime" name="txtIsFirstTime" value="0"/>    
                <?php
                }else{
                ?>                    
                <input type="checkbox" class="expense" id="checkbox<?php echo $i; ?>"
            ...
            <div class="pull-right"
                 style="font-size: 14px; margin-top: 4px;"><?php echo number_format($expense->AMOUNT, 2); ?></div>
        </li>
        <?php
...
<li>
    ...
    <input type="hidden" id="ttl_amt_<?php echo $semester_seq; ?>" value="<?php echo $expense_total; ?>"/><br
        clear="all"/>
</li>
...
?>
<li class="radius0">
    <div class="pull-right <?php echo $prev_amt_class; ?>"><?php echo $label; ?> =
        <strong><?php echo number_format($prevBalance, 2); ?></strong></div>
    <br clear="all"/></li>
<li class="empty_list"></li>
<li class="payment-border radius0">
    <div class="pull-right <?php echo $net_amt_label_class; ?>"><?php echo $net_amt_label; ?> =
        <strong><?php echo number_format($net_amt, 2); ?></strong></div>
    <br clear="all"/></li>