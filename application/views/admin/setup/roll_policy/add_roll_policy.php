    ...
    <form class="form-horizontal frmContent" id="program" method="post">
       <?php
        ...
        <div class="form-group">
            ...
            <div class="col-lg-7">
                <select class="form-control required" name="SESSION_ID" id="SESSION_ID" data-tags="true" data-placeholder="Select Session" data-allow-clear="true">
                    <option value="">Select Session</option>
                    <?php foreach ($session as $row) { ?>
                        <option
                                value="<?php echo $row->SESSION_ID; ?>"><?php echo $row->SESSION_NAME .' - '.$row->DINYEAR; ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            <label class="col-lg-3 pop-width control-label">Program <span class="text-danger">*</span></label>
            <div class="col-lg-7">
                <?php if ($ac_type == "edit"): // if the form action is EDIT ?>
                    <select
                        class="program_dropdown form-control commonClass required"
                        name="PROGRAM_ID" id="PROGRAM_ID" data-tags="true" data-placeholder="Select Program"
                        data-allow-clear="true">
                        <option>Select Program</option>
                        <?php foreach ($program as $row): ?>
                            <option
                                value="<?php echo $row->PROGRAM_ID; ?>" <?php echo ($program->PROGRAM_ID == $row->PROGRAM_ID) ? 'selected' : ''; ?>><?php echo $row->PROGRAM_NAME; ?></option>
                        <?php endforeach; ?>
                    </select>
                <?php else: // if the form action is VIEW ?>
                    <select class="program_dropdown  form-control commonClass required" name="PROGRAM_ID"
         ...
         <div class="hr-line-dashed"></div>
        <div class="form-group">
            <label class="col-lg-3 control-label">Digit<span class="text-danger">*</span></label>
        ...
        <div class="form-group">
            <div class="col-lg-offset-3 col-lg-8">
                ...
                <?php if ($ac_type == "edit") { ?>
                    <input type="button" class="btn btn-primary btn-sm formSubmit" data-action="admission/updateRollPolicy"
                           data-su-action="setup/programById" value="Update">
                <?php } else { ?>
                    <input type="button" class="btn btn-primary btn-sm formSubmit" data-action="admission/createRollPolicy"
