...
<table class="table table-striped table-bordered table-hover gridTable">
    ...
    <tbody>
        <?php if (!empty($grade_policy)): ?>
            <?php $sn = 1; ?>
            <?php foreach ($grade_policy as $row) { ?>
            <tr class="gradeX" id="row_<?php echo $row->GR_POLICY_ID; ?>">
                <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
                    <span><?php echo $sn++; ?></span><span class="hidden"
                    id="loader_<?php echo $row->GR_POLICY_ID; ?>"></span>
                </td>
                ...
                <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
                 ...
                 if ($previlages->DELETE == 1) {
                    ?>
                    <a class="label label-danger deleteItem2" id="<?php echo $row->GR_POLICY_ID; ?>"
                     title="Click For Delete" data-type="delete" data-field="GR_POLICY_ID"
                     data-action="exam/deleteItem"
                     data-tbl="exam_grade_policy"><i
                 ...
                 if ($previlages->STATUS == 1) {
                    ?>
                    <a class="itemStatus2" id="<?php echo $row->GR_POLICY_ID; ?>"
