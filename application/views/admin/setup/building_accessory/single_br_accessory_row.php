<?php if ($previlages->READ == 1) { ?>
    <?php foreach ($br_accessory as $row) { ?>
        <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>><span><?php echo $sn++; ?></span><span
                class="hidden" id="loader_<?php echo $row->SC_BR_ACCESSORY_ID; ?>"></span></td>
        ...
        <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
            <?php if ($previlages->UPDATE == 1) { ?>
                <a class="label label-default openModal" id="<?php echo $row->SC_BR_ACCESSORY_ID; ?>"
                   title="Update Batch Information" data-action="setup/brAccessoryFormUpdate" data-type="edit"><i
            ...
            if ($previlages->STATUS == 1) {
                ...
                <a class="itemStatus" id="<?php echo $row->SC_BR_ACCESSORY_ID; ?>"
                   ...
                   data-field="ACTIVE_STATUS" data-tbl="sc_br_accessories" data-su-url="setup/brAccessoriesById">
