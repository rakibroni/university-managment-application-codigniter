                ...
                <tr class="gradeX" id="row_<?php echo $row->RATE_ID; ?>">
                    <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
                        <span><?php echo $sn++; ?></span><span class="hidden"
                                                               id="loader_<?php echo $row->RATE_ID; ?>"></span></td>
                    ...
                    <td style="width: 140px" <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
                        <a class="label label-info openModal" id="<?php echo $row->RATE_ID; ?>"
                           data-action="setup/expenseInfo" data-type="edit" title="View Expense Information"><i
                                class="fa fa-eye"></i></a>
                        <?php if ($previlages->UPDATE == 1) { ?>
                            <a class="label label-default openModal" id="<?php echo $row->RATE_ID; ?>"
                               title="Update Expense Information" data-action="setup/expenseFormUpdate"
                               data-type="edit"><i class="fa fa-pencil"></i></a>
                        <?php
                        }
                        if ($previlages->DELETE == 1) {
                            ?>
                            <a class="label label-danger deleteItem" id="<?php echo $row->RATE_ID; ?>"
                               title="Click For Delete" data-action="setup/deleteDegree" data-type="delete"
                               data-field="RATE_ID" data-tbl="ac_academic_charge_rate"><i class="fa fa-times"></i></a>
                        <?php
                        }
                        if ($previlages->STATUS == 1) {
                            ?>
                            <a class="itemStatus" id="<?php echo $row->RATE_ID; ?>"
                               data-status="<?php echo $row->ACTIVE_STATUS ?>" data-fieldId="RATE_ID"
                               data-field="ACTIVE_STATUS" data-tbl="ac_academic_charge_rate"
