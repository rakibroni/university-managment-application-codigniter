        ...
        <form id="applicant"> 
            <div class="col-md-6">
                <div class="form-group"> 
                    <select class="form-control required" name="PROGRAM_ID" id="PROGRAM_ID" data-tags="true" data-placeholder="Select Program" data-allow-clear="true">
                    ...
                    <table class="table table-striped table-bordered table-hover gridTable">
                        ...
                        <tbody>
                         ...
                         <td>
                            ...
                            else if($row->ELIGIBLE_BY_ADDMISSION_DEPT_STATUS == '2')
    ...
    <script type="text/javascript">
        ...
        $(document).on("click", ".formApproveSubmit", function () {
            if (confirm("Are You Sure?")) {
                ...
                $.ajax({
                    type: "post",
                    url: '<?php echo site_url('admin/newApplicantApproved') ?>',
                    ...
                    success: function (data) {
                        ...
                        $.ajax({
                            type: "post",
                            data: {applicantId: applicantId},
                            url: "<?php echo site_url(); ?>/admin/applicantById",
        ...
        $(document).on("click", ".formRejectSubmit", function () {
            if (confirm("Are You Sure?")) {
                ...
                $.ajax({
                    type: "post",
                    data: {remarks:remarks, applicantId:applicantId},
                    url: '<?php echo site_url('admin/newApplicantReject') ?>/',
                    success: function (data) {
                        $(".frmMsg").html(data); 
                        $.ajax({
                            type: "post",
        ...
        $(document).on("click", ".program_session_wise_applicant", function () {
            ...
            if(PROGRAM_ID =='' || YSESSION_ID=='' ){
                ...
                $.ajax({
                    ...
                    url: "<?php echo site_url()?>/Admin/newApplicantProSesWise",
                    beforeSend: function () {
        ...
        $(document).on("click", ".allApprove", function () {
            ...
            if (!$('#frmContent input[type="checkbox"]').is(':checked')) {
                ...
                if (confirm("Are You Sure?")) {
                    ...
                    var offer = $("#OFFER_TYPE").val();
                    $.ajax({
                        type: "POST",
                        url: '<?php echo site_url('admin/allApplicantApproved') ?>',
                        data: frmContent,
                        beforeSend: function () {
                        ...
                        success: function (data) {
                            if(data == "appp"){
                                alert("approve don't insert. Please program particulars setup first");
                            }else {
                                $(".loadingImg").html("");
                                $('#upgrade_success').html(data);
                                $.ajax({
                                    type: "post",
                                    data:{program:program, session:session, offer:offer},
                                    url: '<?php echo site_url('admin/approveApplicantList') ?>/',
                                    success: function (data1) {
                                        $(".loadingImg").html("");
                                        $('#approveApplicant').html(data1);                            
                                    }
                                });                            
                            }
                        }
        ...
        $(document).ready(function () {
            $(document).on("click", ".formOffer", function () {
                ...
                if (is_valid == 0) {
                    ...
                    var action_url = '<?php echo site_url('admin/searchNewPassedApplicant') ?>';
                    $.ajax({
                        type: "POST",
                        url: action_url,
                        data: {
