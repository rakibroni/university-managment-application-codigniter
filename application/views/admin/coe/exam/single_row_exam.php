<?php if ($previlages->READ == 1) { ?>
    <?php foreach ($exam as $row) { ?>
        <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>><span><?php echo $sn++; ?></span><span
                class="hidden" id="loader_<?php echo $row->EXAM_ID; ?>"></span></td>
        ...
        <td style="width: 140px" <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
            ...
            <?php if ($previlages->UPDATE == 1) { ?>
                <a class="label label-default openModal" id="<?php echo $row->EXAM_ID; ?>" title="Update Exam"
                   data-action="Coe/examFormUpdate" data-type="edit"><i class="fa fa-pencil"></i></a>
            <?php
            }
            if ($previlages->DELETE == 1) {
                ?>
                <a class="label label-danger deleteItem" id="<?php echo $row->EXAM_ID; ?>" title="Click For Delete"
                   data-type="delete" data-field="EXAM_ID" data-tbl="exam"><i class="fa fa-times"></i></a>
            <?php
            }

            if ($previlages->STATUS == 1) {
                ?>
                <a class="itemStatus" id="<?php echo $row->EXAM_ID; ?>" data-status="<?php echo $row->ACTIVE_STATUS ?>"
                   data-fieldId="EXAM_ID" data-field="ACTIVE_STATUS" data-tbl="exam" data-su-url="Coe/examById">
