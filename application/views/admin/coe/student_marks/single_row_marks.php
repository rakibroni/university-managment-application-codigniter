<?php if ($previlages->READ == 1) { ?>
    <?php foreach ($marks as $row) { ?>
        <td <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>><span><?php echo $sn++; ?></span><span
                class="hidden" id="loader_<?php echo $row->EX_MARKS_ID; ?>"></span></td>
        ...
        <td style="width: 140px" <?php echo ($row->ACTIVE_STATUS == 1) ? "" : "class='inactive'"; ?>>
            ...
            if ($previlages->DELETE == 1) {
                ?>
                <a class="label label-danger deleteItem" id="<?php echo $row->EX_MARKS_ID; ?>" title="Click For Delete"
                   data-type="delete" data-field="EX_MARKS_ID" data-tbl="exam_student_marks"><i class="fa fa-times"></i></a>
            <?php
            }
            if ($previlages->STATUS == 1) {
                ?>
                <a class="itemStatus" id="<?php echo $row->EX_MARKS_ID; ?>"
                   data-status="<?php echo $row->ACTIVE_STATUS ?>" data-fieldId="EX_MARKS_ID" data-field="ACTIVE_STATUS"
                   data-tbl="exam_student_marks" data-su-url="Coe/studentMarksById">
