                ...
                foreach ($group_modules as $group_module):
                    ...
                    <tr title="<?php echo $group_module->SA_MODULE_NAME; ?>">
                        <th class="bg-font-normal" colspan="50"
                            style="background: url('<?php echo base_url(); ?>resources/img/header_bg.jpg'); padding: 3px; color: #427DAE;"><?php echo $group_module->SA_MODULE_NAME; ?></th>
                    </tr>
                    <?php
                            ...
                            foreach ($levels as $level):
                                ...
                                <td class="text-center">
                                    <input type="checkbox" class="assignMlink"
                                           title="<?php echo "Group: $level->UGLEVE_NAME, Page: $org_module_link->LINK_NAME"; ?>" <?php echo ($this->utilities->level_is_it_checked_or_not($level->UG_LEVEL_ID, $org_module_link->SA_MLINKS_ID) == TRUE) ? 'checked="checked"' : ''; ?>
                                           value="<?php echo "$level->UG_LEVEL_ID,$org_module_link->SA_MLINKS_ID"; ?>"
                                           id="<?php echo $org_module_link->USERGRP_ID; ?>"
                                           rel="<?php echo $group_module->SA_MODULE_ID; ?>"/>
                                </td>
                            <?php endforeach; ?>
                        </tr>
                    <?php endforeach; ?>
