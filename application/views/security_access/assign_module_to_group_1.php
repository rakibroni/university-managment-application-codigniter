    ...
    <div class="span12">
        <div class="widget green">
            <div class="widget-title">
                ...
                <a href="<?php echo site_url('cp/securityAccess/allGroup'); ?>" class="label label-important"><span
                        class="icon-eye-open"></span> View All Groups</a>
                <span class="tools">
                ...
                <table class="table table-striped table-bordered nestedTable" id="sample_1">
                    ...
                    <?php foreach ($org_modules as $org_module): ?>
                        <tr title="<?php echo $org_module->SA_MODULE_NAME; ?>">
                            <th class="bg-font-normal" colspan="50"
                                style="background: url('<?php echo base_url(); ?>resources/img/header_bg.jpg'); padding: 3px; color: #427DAE;"><?php echo $org_module->SA_MODULE_NAME; ?></th>
                        </tr>
                        <?php
                        ...
                        foreach ($org_module_links as $org_module_link):
                            ...
                            <tr title="<?php echo $org_module_link->LINK_NAME; ?>">
                                <td class="bg-font-normal"
                                    style="text-align: left;"><?php echo $org_module_link->LINK_NAME; ?></td>
                                <?php
                                foreach ($groups as $group):
                                    ?>
                                    <td class="text-center">
                                        <input type="checkbox" class="assignMlink"
                                               title="<?php echo "Group: $group->USERGRP_NAME, Page: $org_module_link->LINK_NAME"; ?>" <?php echo ($this->utilities->is_it_checked_or_not($group->USERGRP_ID, $org_module_link->SA_MLINKS_ID) == TRUE) ? 'checked="checked"' : ''; ?>
                                               value="<?php echo "$group->USERGRP_ID,$org_module_link->SA_MLINKS_ID"; ?>"
                                               id="<?php echo $org_module_link->LINK_ID; ?>"
                                               rel="<?php echo $org_module->SA_MODULE_ID; ?>"/>
                                    </td>
                                <?php endforeach; ?>
                            </tr>
                        <?php endforeach; ?>
